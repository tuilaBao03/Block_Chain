...
class Peer(DatagramProtocol):   
    ...
    def startProtocol(self):
        self.transport.write(msgpack.packb("sẵn sàng"), self.server)
    
    def datagramReceived(self, datagram, addr): 
        holder_public = ""
        mini_public_address = ""
        datagram = msgpack.unpackb(datagram) # Convert received data to string
        if addr == self.server:
    ...
    def send_message(self): 
        while True:
            ...
            if message.lower() == 'ccdc': # cung cap dai chi public cua cong // truyền địa chỉ cho máy gửi để nhận tiền 
                ...
                self.transport.write(msgpack.packb(canchuyen), self.remote_address)
                print("da gui dia chi")
            else:
                self.transport.write(msgpack.packb(message), self.remote_address)
                print("da gui")
